...
import { handleDeleteTermsCondition } from "@/lib/actions/admin/termscondition-action";


const TermsConditionTable = ({ termsconditions, pagination, search }: { termsconditions: any[], pagination: any, search?: string }) => {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = useState(search || '');
  const [deleteId, setDeleteId] = useState<string | null>(null);

  const handleSearchChange = () => {
    router.push(`/admin/termsconditions?page=1&size=${pagination.size}` +
      (searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : ''));
  };

  const onDelete = async () => {
    try {
      await handleDeleteTermsCondition(deleteId!);
      toast.success("Terms Condition deleted successfully");
    } catch (err: any) {
      toast.error(err.message || "Failed to delete Terms Condition");
    } finally {
      setDeleteId(null);
    }
  };
  ...
  const makePagination = (): React.ReactElement[] => {
    ...
    const prevHref = `/admin/termsconditions?page=${currentPage - 1}&size=${pagination.size}` +
      (searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : '');
    pages.push(
      <button
        key="prev"
    ...
    if (startPage > 1) {
      pages.push(
        <button key={1} onClick={() => router.push(`/admin/termsconditions?page=1&size=${pagination.size}`)} className="px-3 py-1 border rounded-md bg-white text-orange-400 hover:bg-orange-100">1</button>
      );
      if (startPage > 2) pages.push(<span key="ellipsis1" className="px-2">...</span>);
    }

    for (let i = startPage; i <= endPage; i++) {
      const href = `/admin/termsconditions?page=${i}&size=${pagination.size}${searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : ''}`;
      pages.push(
    ...
    if (endPage < totalPages) {
      ...
      pages.push(
        <button key={totalPages} onClick={() => router.push(`/admin/termsconditions?page=${totalPages}&size=${pagination.size}`)} className="px-3 py-1 border rounded-md bg-white text-orange-500 hover:bg-orange-100">{totalPages}</button>
      );
    }

    const nextHref = `/admin/termsconditions?page=${currentPage + 1}&size=${pagination.size}${searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : ''}`;
    pages.push(
  ...
  return (
    <div className="mt-6 border border-gray-200 rounded-lg overflow-hidden bg-white">
      ...
      {/* <DeleteModal
        ...
        description="Are you sure you want to delete this TermsCondition? This action cannot be undone."
      /> */}
      ...
      <div className="p-4 flex items-center border-b border-gray-200 gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by Terms Condition title"
      ...
      <table className="w-full table-auto border-collapse">
        ...
        <tbody className="divide-y divide-gray-200 text-gray-700">
          {termsconditions.map(termsconditions => (
            <tr key={termsconditions._id} className="hover:bg-gray-50 font-inter font-medium text-gray-700">
             
              <td className="px-4 py-2">{termsconditions.title}</td>
              <td className="px-4 py-2">{termsconditions.description}</td>
...
// // //                                 <Link href={`/admin/users/${user._id}/edit`} className="text-blue-500 ml-4 hover:underline">Edit</Link> */}
                ...
                {/* <div className="flex flex-col items-center gap-1">
                 ...
                 {/* View */}
                 <Link href={`/admin/termsconditions/${termsconditions._id}`} className="flex flex-col items-center gap-1">
                 <FiEye className="text-gray-500 text-lg" />
                 <span className="text-green-600 text-xs">View</span>
                 </Link>
                 {/* Edit */}
                 <Link href={`/admin/termsconditions/${termsconditions._id}/edit`} className="flex flex-col items-center gap-1">
                 <FiEdit className="text-gray-500 text-lg" />
                 <span className="text-blue-500 text-xs">Edit</span>
                 </Link>
                {/* Delete */}
                <div className="flex flex-col items-center gap-1 cursor-pointer" onClick={() => setDeleteId(termsconditions._id)}>
